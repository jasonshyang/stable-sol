...
import { StableSol } from "../target/types/stable_sol";
...
describe("stablesol", () => {
  ...
  const program = anchor.workspace.StableSol as Program<StableSol>;
  ...
  it("Withdraw Collateral and Burn USDS", async () => {
    const amountCollateral = 500_000_000;
    const amountToBurn = 500_000_000;
    const tx = await program.methods
      .withdrawCollateralAndBurnTokens(
        new anchor.BN(amountCollateral),
  ...
  it("Liquidate", async () => {
    ...
    const tx = await program.methods
      ...
      .accounts({ collateralAccount, priceUpdate: solUsdPriceFeedAccount }) // need the account to be specified as it is not a signer
      .rpc({ skipPreflight: true, commitment: "confirmed" });
    console.log("Your transaction signature", tx);
  });
});
